...
export type SafeUser = { id: string; fullName: string; whatsapp: string; email: string | null; studentId: string | null; role: "STUDENT" | "SUPER_ADMIN"; status: "PENDING" | "APPROVED" | "REJECTED" | "SUSPENDED" };
type DbUser = { id: string; full_name: string; whatsapp_number: string; email: string | null; student_id: string | null; password_hash: string; role: SafeUser["role"]; status: SafeUser["status"]; last_login_at: string | null };
function safeUser(user: DbUser): SafeUser { return { id: user.id, fullName: user.full_name, whatsapp: user.whatsapp_number, email: user.email, studentId: user.student_id, role: user.role, status: user.status }; }
function normalizeWhatsApp(value: string) { return value.replace(/[\s()-]/g, ""); }
function cookieHeader(token: string, expiresAt: Date) { const secure = typeof process !== "undefined" && process.env.NODE_ENV === "production" ? "; Secure" : ""; return `${COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax${secure}; Expires=${expiresAt.toUTCString()}`; }
export function clearSessionCookie() { const secure = typeof process !== "undefined" && process.env.NODE_ENV === "production" ? "; Secure" : ""; return `${COOKIE}=; Path=/; HttpOnly; SameSite=Lax${secure}; Max-Age=0`; }
export function getCookie(request: Request) { const raw = request.headers.get("cookie") || ""; const match = raw.split(";").map((v) => v.trim()).find((v) => v.startsWith(`${COOKIE}=`)); return match ? decodeURIComponent(match.slice(COOKIE.length + 1)) : null; }
...
export function enforceRateLimit(request: Request, keySuffix: string, max = 12, windowMs = 15 * 60_000) {
  ...
  const key = `${keySuffix}:${ip}`; const now = Date.now(); const current = attempts.get(key);
...
export async function requireAuth(request: Request) {
  enforceSameOrigin(request); const user = await getSessionUser(request);
...
export async function requireAdmin(request: Request) { const user = await requireAuth(request); if (user.role !== "SUPER_ADMIN" || user.status !== "APPROVED") throw new Response(JSON.stringify({ error: "Forbidden" }), { status: 403, headers: { "Content-Type": "application/json" } }); return user; }
export async function destroySession(request: Request) { const token = getCookie(request); if (token) await db().from("auth_sessions").delete().eq("token_hash", await sessionHash(token)); }
...
export async function registerStudent(input: z.infer<typeof registerSchema>) {
  const parsed = registerSchema.parse(input); const whatsapp = normalizeWhatsApp(parsed.whatsapp); const studentId = parsed.studentId || null; const email = parsed.email || null; const client = db();
...
export async function loginStudent(whatsappInput: string, password: string, remember: boolean) {
  const whatsapp = normalizeWhatsApp(whatsappInput); const { data } = await db().from("auth_users").select("*").eq("whatsapp_number", whatsapp).eq("role", "STUDENT").maybeSingle();
  ...
  await db().from("auth_users").update({ last_login_at: new Date().toISOString() }).eq("id", data.id); return createSession(data.id, remember);
}
export async function loginAdmin(whatsappInput: string, password: string, remember: boolean) {
  const whatsapp = normalizeWhatsApp(whatsappInput); const configured = env("ADMIN_WHATSAPP");
  ...
  const client = db(); const existing = await client.from("auth_users").select("id").eq("whatsapp_number", whatsapp).eq("role", "SUPER_ADMIN").maybeSingle(); let userId = existing.data?.id as string | undefined;
  if (!userId) { const { data, error } = await client.from("auth_users").insert({ full_name: "Super Admin", whatsapp_number: whatsapp, password_hash: env("ADMIN_PASSWORD_HASH"), role: "SUPER_ADMIN", status: "APPROVED" }).select("id").single(); if (error) throw new Error(error.message); userId = data.id; }
  await client.from("auth_users").update({ status: "APPROVED", password_hash: env("ADMIN_PASSWORD_HASH"), last_login_at: new Date().toISOString() }).eq("id", userId); return createSession(userId, remember);
}
export function sessionResponse(body: unknown, token: string, expiresAt: Date, status = 200) { return new Response(JSON.stringify(body), { status, headers: { "Content-Type": "application/json", "Set-Cookie": cookieHeader(token, expiresAt) } }); }
...
export async function adminStudents() { const { data, error } = await db().from("auth_users").select("id,full_name,whatsapp_number,email,student_id,role,status,created_at,last_login_at").eq("role", "STUDENT").order("created_at", { ascending: false }); if (error) throw new Error(error.message); return (data || []).map((u) => ({ id: u.id, fullName: u.full_name, whatsapp: u.whatsapp_number, email: u.email, studentId: u.student_id, status: u.status, createdAt: u.created_at, lastLoginAt: u.last_login_at })); }
export async function updateStudentStatus(id: string, status: "PENDING" | "APPROVED" | "REJECTED" | "SUSPENDED") { const { data, error } = await db().from("auth_users").update({ status }).eq("id", id).eq("role", "STUDENT").select("id,status").maybeSingle(); if (error || !data) throw new Error(error?.message || "Student not found"); if (status !== "APPROVED") await invalidateUserSessions(id); return data; }
