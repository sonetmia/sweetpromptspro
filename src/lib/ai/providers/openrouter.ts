...
export class OpenRouterProvider implements AIProviderAdapter {
  ...
  description = 'Unified gateway to 100+ global models including Claude 3.5 Sonnet, GPT-4o, and DeepSeek.';
  capabilities = PROVIDER_CAPABILITIES.openrouter;

  detectKey(apiKey: string): boolean {
    const trimmed = apiKey.trim();
    return trimmed.startsWith('sk-or-v1-') || (trimmed.startsWith('sk-') && trimmed.includes('or'));
  }
    ...
    try {
      ...
      if (!data.success) {
        return {
          success: false,
          provider: this.id,
          message: data.error || data.message || 'OpenRouter connection failed',
          error: data.error,
          hasText: false,
          hasVision: false,
      ...
      return {
        success: true,
        provider: this.id,
        message: data.message || 'Connected to OpenRouter Gateway successfully',
        hasText: true,
        hasVision: true,
        modelsCount: this.capabilities.supportedModels.length,
        availableModels: this.capabilities.supportedModels,
      ...
      return {
        success: false,
        provider: this.id,
        message: err.message || 'Network error during OpenRouter connection test',
        error: err.message,
        hasText: false,
        hasVision: false,
        modelsCount: this.capabilities.supportedModels.length,
  ...
  async listModels(_config?: ProviderConfig): Promise<AIModelInfo[]> {
    return this.capabilities.supportedModels.map(m => ({
      id: m,
      name: m,
      description: m.includes('claude') ? 'Anthropic Claude flagship creative model' : m.includes('gpt-4o') ? 'OpenAI GPT-4o multimodal' : 'High performance reasoning model',
      supportsVision: m.includes('claude') || m.includes('gpt-4o') || m.includes('flash'),
  ...
  async generateStructured<T = any>(req: GenerateStructuredRequest, config?: ProviderConfig): Promise<T> {
    ...
    try {
      ...
      throw new Error(`Failed to parse structured response from OpenRouter: ${rawResult.slice(0, 100)}`);
    }
  }
}
