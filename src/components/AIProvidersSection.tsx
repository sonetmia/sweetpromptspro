...
const REGISTRY: ProviderMeta[] = [
  ...
  },
  {
    ...
    async listModels(key, extra) {
      ...
      const r = await fetch(
        `https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(extra)}/ai/models/search?per_page=100`,
        { headers: bearer(key) },
      );
      const j = await jsonOrThrow(r);
  ...
  },
  {
    ...
    async listModels(key) {
      const r = await fetch("https://models.github.ai/catalog/models", {
        headers: { ...bearer(key), Accept: "application/vnd.github+json" },
      });
      const j = await jsonOrThrow(r);
  ...
  },
  {
    ...
    async listModels(key) {
      const r = await fetch("https://api.cohere.com/v1/models?page_size=100", {
        headers: bearer(key),
      });
      const j = await jsonOrThrow(r);
    ...
    async testConnection(key) {
      const r = await fetch("https://api.cohere.com/v1/models?page_size=1", {
        headers: bearer(key),
      });
      await jsonOrThrow(r);
    },
  ...
  try {
    ...
    if (raw) {
      ...
      for (const meta of REGISTRY)
        providers[meta.id] = { ...emptyState(), ...(parsed.providers?.[meta.id] ?? {}) };
      return {
        version: 1,
...
export function AIProvidersSection({ C }: { C: Theme }) {
  ...
  const persist = (next: ConfigShape) => {
    setCfg(next);
    saveConfig(next);
    flash();
  };
  ...
  const resetAll = () => {
    if (!confirm("Reset all AI provider configuration? Keys and cached models will be cleared."))
      return;
    const providers = {} as Record<ProviderId, ProviderState>;
    for (const meta of REGISTRY) providers[meta.id] = emptyState();
    ...
    <div>
      ...
      >
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              persist({ ...cfg, defaultProvider: e.target.value as ProviderId | "" })
            }
            style={{
              width: "100%",
              background: C.card,
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 10.5, color: C.muted }}>
              Try next enabled provider on failure
            </div>
          </div>
        </label>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
          <MiniBtn C={C} onClick={saveAll} tone="orange">
            💾 Save All
          </MiniBtn>
          <MiniBtn C={C} onClick={resetAll} tone="red">
            ↺ Reset
          </MiniBtn>
          <MiniBtn C={C} onClick={exportCfg}>
            ⇩ Export
          </MiniBtn>
          <MiniBtn C={C} onClick={() => importRef.current?.click()}>
            ⇪ Import
          </MiniBtn>
          <input
            ref={importRef}
        ...
        {saved && (
          <div style={{ gridColumn: "1 / -1", fontSize: 11.5, color: C.green, fontWeight: 600 }}>
            ✓ Saved
          </div>
        )}
      </div>
      ...
      >
        ...
        <p style={{ fontSize: 12, color: C.muted, lineHeight: 1.65, margin: 0 }}>
          API keys are stored locally in your browser only (localStorage). Exported configs include
          keys — share carefully. Test calls go directly from your browser to the provider; some
          providers may block browser requests via CORS, in which case you can still save the key
          and use manual model entry.
        </p>
      </div>
    </div>
  );
...
function ProviderCard({
  ...
  const keyFormatOk =
    !state.apiKey.trim() || !meta.keyPattern || meta.keyPattern.test(state.apiKey.trim());
  const extraMissing = !!meta.extraRequired && !state.extra.trim();

  const runTest = async () => {
    if (keyEmpty) {
      onChange({ lastTest: "invalid", lastTestMsg: "API key is empty", lastTestedAt: Date.now() });
      return;
    }
    if (extraMissing) {
      onChange({
        lastTest: "invalid",
        lastTestMsg: `${meta.extraLabel} is required`,
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      onChange({
        lastTest: kind,
        lastTestMsg: "Model load failed: " + labelFor(kind),
        lastTestedAt: Date.now(),
      });
    } finally {
    ...
    >
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          />
          <IconBtn C={C} onClick={() => setShowKey((s) => !s)} title={showKey ? "Hide" : "Show"}>
            {showKey ? "🙈" : "👁"}
          </IconBtn>
          <IconBtn C={C} onClick={copyKey} title="Copy" disabled={keyEmpty}>
            {copied ? "✓" : "⧉"}
          </IconBtn>
          <IconBtn
            C={C}
            onClick={() => onChange({ apiKey: "", lastTest: "", lastTestMsg: "" })}
            title="Clear"
            disabled={keyEmpty}
          >
            ✕
        ...
        {!keyFormatOk && (
          <div style={{ fontSize: 10.5, color: C.red, marginTop: 4 }}>
            Key format looks unusual for {meta.name}.
          </div>
        )}
        <a
          href={meta.docsUrl}
      ...
      {meta.extraLabel && (
        <div>
          <FieldLabel C={C}>
            {meta.extraLabel}
            {meta.extraRequired && <span style={{ color: C.red }}> *</span>}
          </FieldLabel>
          <input
            type="text"
            value={state.extra}
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <IconBtn
            ...
            disabled={keyEmpty || loadingModels}
          >
            {loadingModels ? "…" : "↻"}
          </IconBtn>
        </div>
                ...
                >
                  {filteredModels.map((m) => (
                    <button
                      ...
                      style={{
                        textAlign: "left",
                        background: state.model === m ? C.orangeSoft : "transparent",
                  ...
                  {filteredModels.length === 0 && (
                    <div style={{ fontSize: 11, color: C.dim, padding: 4 }}>No matches</div>
                  )}
                </div>
              </div>
            )}
      ...
      >
        ...
        <span style={{ fontSize: 11.5, color: statusColor, fontWeight: 600 }}>
          {state.lastTest === ""
            ...
            : state.lastTest === "ok"
              ? "✓ Connected"
              : state.lastTest === "invalid"
                ? "✗ Invalid API Key"
                : state.lastTest === "network"
                  ? "✗ Network Error"
                  : state.lastTest === "ratelimit"
                    ? "✗ Rate Limited"
                    : "✗ Error"}
        ...
        <button
          ...
          style={{
            ...
            background:
              testing || keyEmpty
                ? C.card2
                : `linear-gradient(180deg, ${C.blue} 0%, ${C.blue}dd 100%)`,
