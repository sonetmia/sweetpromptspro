...
const C = new Proxy({} as Theme, {
  get(_, p: string) {
    return CURRENT[p as keyof Theme];
  },
});
...
function ExportBar({ prompts }: { prompts: string[] }) {
  ...
  const doDownload = (fmt: "txt" | "csv") => {
    ...
    if (fmt === "txt") dlTxt(prompts);
    else dlCsv(prompts);
  };
  const confirmDl = () => {
    if (modal) {
      if (modal.format === "txt") dlTxt(prompts);
      else dlCsv(prompts);
      setModal(null);
    }
  };
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && (
        ...
        >
          {items.map((t: { id: string; label: string; icon: string }) => (
            <button
              key={t.id}
              onClick={() => {
                setPage(t.id);
...
function PageContent({ page, themeKey, setThemeKey }: unknown) {
