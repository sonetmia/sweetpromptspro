...
export default function MetadataStudio() {
  const [images, setImages] = useState<
    { file: File; dataUrl: string; result?: unknown; loading?: boolean; error?: string }[]
  >([]);
  ...
  const handleFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages: { file: File; dataUrl: string; error?: string }[] = [];
    for (const f of newFiles) {
      try {
        newImages.push({ file: f, dataUrl: await fileToValidatedDataUrl(f) });
      } catch (err) {
        newImages.push({ file: f, dataUrl: "", error: aiErrorMessage(err) });
      }
    }
  ...
  const processImage = async (index: number) => {
    ...
    try {
      ...
      setImages((prev) => {
        const next = [...prev];
        next[index] = { ...next[index], loading: false, error: aiErrorMessage(e) };
        return next;
      });
    }
  };
